...
I wrote a backup\_tool.c that creates a log file named backup.log, reads the contents of source.txt, copies them into backup.txt, and records the backup process in the log file.
...
*gcc backup\_tool.c \-o backup\_tool*
...
*strace \-o trace.txt ./backup\_tool*
...
* trace.txt should contain the system calls made during execution, including file operations such as openat, read, write, and close, as well as process-related calls such as execve and exit\_group.

## **System Call Summary Table**

| Syscall | Category | Purpose |
| :---- | :---- | :---- |
| execve | Process | Loads and starts the backup\_tool program |
...
| arch\_prctl / set\_tid\_address / set\_robust\_list / rseq | Process (init) | Low-level thread/process setup performed automatically by glibc at startup |
...
| exit\_group | Process | Terminates the process once the backup is complete |
